...
router.get(
  ...
  h(async (req, res) => {
    ...
    const rows = emps.map((e) => {
      ...
      const cells = {};
      const summary = { T: 0, TA: 0, P: 0, L: 0, LE: 0, NA: 0 };
      days.forEach((day) => {
        if (!activeOn(e, day)) {
          cells[day] = { code: null, applicable: false, locked: false };
          summary.NA += 1;
          return;
        }
        const m = marked[day];
...
router.get(
  ...
  h(async (req, res) => {
    ...
    const buf = await buildWorkbook({
      ...
      columns,
      rows,
      notes: [`Codes: ${CODES.map((c) => `${c} = ${CODE_LABEL[c]}`).join('   |   ')}`,
        'Blank cells fall outside the driver\'s deployment period.'],
    });
